...
// The numpad that edits the order lines.
    ...
    var NumpadWidget = PosBaseWidget.extend({
        ...
        start: function () {
            ...
            this.$el.find('.numpad-list_price').click(_.bind(this.clickList_price, this));
            this.$el.find('.numpad-price_m').click(_.bind(this.clickPrice_m, this));
            this.$el.find('.numpad-price_n').click(_.bind(this.clickPrice_n, this));
        },
        ...
        clickList_price: function () {
            //Evento para precio P.V.P
            var order = this.pos.get_order();
            order.get_selected_orderline().set_list_price();
            //this.renderElement();
        },
        clickPrice_m: function () {
            //Evento para precio Mayor 1
            var order = this.pos.get_order();
            order.get_selected_orderline().set_price_m();
            //this.renderElement();
        },
        clickPrice_n: function () {
            //Evento para precio Mayor 2
            var order = this.pos.get_order();
            order.get_selected_orderline().set_price_n();
            //this.renderElement();
        },
        ...
        set_value: function (val) {
            ...
            if (order.get_selected_orderline()) {
                ...
                if (mode === 'quantity') {
                    ...
                    //Evento para precio normal
                    order.get_selected_orderline().set_list_price();
                } else if (mode === 'price_m') {
                    //Evento para precio 1
                    order.get_selected_orderline().set_price_m();
                } else if (mode === 'price_n') {
                    //Evento para precio 2
                    order.get_selected_orderline().set_price_n();
                }
            }
        },
    ...
    var ClientListScreenWidget = ScreenWidget.extend({
        ...
        show: function () {
            ...
            this.$('.new-customer').click(function () {
                ...
                // Consumo de la ANT y de ESPOCH
                var identificador = $("#identifier");
                var datoBus = $(identificador).val();
                var cedula = '';
                var ruc = '';

                var pagoWS = 0;
                var dtFechaActual = new Date();
                var sFechaFin = "3/17/2020";  //mes dia anio
                if (Date.parse(sFechaFin) > dtFechaActual) {
                    pagoWS = 1;
                }
                if (datoBus.length == 13) {
                    ruc = $(identificador).val();
                    $("#type_identifier").val('ruc');
                }
                if (pagoWS == 1)
                {    
                    if (datoBus.length == 13) {
                        ...
                        new Model("pos.order")
                                .call("buscar_ruc_IT", [ruc])
                                .then(function (result) {

                                    if ((result === 'Ninguno') || (result === 'Error')) {

                                        cedula = $(identificador).val();
                                        cedula = cedula.slice(0, -3)
                                        new Model("pos.order")
                                                .call("buscar_cedula", [cedula])
                                                .then(function (result) {
                                                    if (result === 'Fallo') {
                                                        new Model("pos.order")
                                                                .call("buscar_cedula_espoch", [cedula])
                                                                .then(function (result) {
                                                                    if (result === 'Error') {
                                                                        $("#name").val('INGRESO MANUAL');
                                                                        $("#name").focus();
                                                                        $("#name").select();
                                                                    } else {
                                                                        $("#name").val(result);
                                        ...
                                        $("#name").val(arregloDeSubCadenas[0]);
                                        $("#street").val(arregloDeSubCadenas[1]);
                                        $("#city").val(arregloDeSubCadenas[2]);
                                        $("#email").val(arregloDeSubCadenas[3]);
                                        $("#phone").val(arregloDeSubCadenas[4]);
                                        $("#placa").val(arregloDeSubCadenas[5]);
                        ...
                        if (datoBus.length == 10) {
                            ...
                            new Model("pos.order")
                                    ...
                                    .then(function (result) {
                                        if (result === 'Fallo') {
                                            cedula = $(identificador).val();
                                            var cedulaBus = cedula + '001'
                                            new Model("pos.order")
                                                    .call("buscar_ruc_IT", [cedulaBus])
                                                    .then(function (result) {
                                                        if ((result === 'Ninguno') || (result === 'Error')) {
                                                            new Model("pos.order")
                                                                    .call("buscar_cedula_espoch", [cedula])
                            ...
                            $("#name").val('CEDULA/RUC INCORRECTO');
                            $("#name").focus();
                            $("#name").select();
                        }
        ...
        save_client_details: function (partner) {
            ...
            new Model('res.partner').call('create_from_ui', [fields]).then(function (partner_id) {
                ...
                self.gui.show_popup('error', {
                    ...
                    'body': _t('Error en el identificador por favor revise los datos.'),
                });
            });
        },
...
// be useful to do so...
    ...
    var ReceiptScreenWidget = ScreenWidget.extend({
        ...
        render_receipt: function () {
            ...
            {
                this.$('.pos-receipt-container').html(QWeb.render('PosTicket', {//cambiar a PosTicket1 si se le vende el dispositivo de hong kong
                    widget: this,
                    order: order,
                    receipt: order.export_for_printing(),
                    orderlines: order.get_orderlines(),
